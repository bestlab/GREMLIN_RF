...
#HOME = os.getenv('HOME')
#DATA_DIR = "%s/sw/scratch_cvs/go_builder"%HOME
...
std_sigrep = 3.0
# WHY WAS std_bb_sigrep SET TO THIS 6.0? WAS THAT FOR NUCLEIC ACIDS?
# NEED TO CHECK!!! THIS VALUE IS TOO BIG FOR PROTEINS
#std_bb_sigrep = 6.0
std_bb_sigrep = 3.0
std_sc_sigrep = 3.0
hcut_theta = 135.0
...
def read_pasta_parallel():
	"""read in and parse parallel-beta pasta parameters
	REF: Trovato et al, PLoS Comput Biol, v2, p1608-1618 [e170] (2006)"""
	pdat = map(lambda x: x.split(), open("%s/pasta_parallel.dat"%(DATA_DIR)).readlines())
	pmap = {}
	for d in pdat:
		i,j,eij_kT = d[0],d[1],float(d[2])
		if i not in pmap.keys():
			pmap[i] = {}
		if j not in pmap.keys():
			pmap[j] = {}
		pmap[i][j] = eij_kT 
		pmap[j][i] = eij_kT
	return pmap
...
def calc_pasta_intra(crda,E0=0.0,LAMBDA=1.0):
	"""Calculate kh matrix of interactions within a chain"""
	khmap = {}
	nresa = len(crda.keys())
	mjmap = {}
	mjmap = read_pasta_parallel()
	for i in range(1,nresa+1):
		for j in range(i,nresa+1):
			itype = crda[i]["name"]
			jtype = crda[j]["name"]
			...
			# hack
			#if i == j:
			#	epsij = LAMBDA*(mjmap[itype][jtype]-E0) * 300.*8.31/4184. # to get kcal/mol from kT
			#	#epsij = LAMBDA*(-1.0) # hard code to 1 kcal/mol (x lambda) for self interactions
			#	if abs(epsij) < 1e-20:	# i.e. zero! - would be no pair interaction calculated
			#		epsij = 1.e-4	# introduce small hard-core repulsion
			#else:
			#	epsij = 1.e-4	# introduce small hard-core repulsion
...
def calc_pasta_inter(crda,crdb,E0=0.0,LAMBDA=1.0):
	"""Calculate kh matrix of interactions between two chains"""
	khmap = {}
	nresa = len(crda.keys())
	nresb = len(crdb.keys())
	...
	for i in range(1,nres+1):
		...
		for j in range(i,nres+1):
			...
			if (i,j) in nbfix.keys():
				...
				C12 = def_C12
				C6 = eij
				otop.write("G%-5i G%-5i 1  %14.6e %14.6e\n" \
					% (i,j,C6,C12))
