...
Usage:
	...
	[-O] [--fixrep] [-p] [-s seq] [-H] [-c A:B:C] [-q] [--kh] [--pasta]
	...
	in which:
		...
		--pasta: introduces "pasta" potential for parallel beta contacts
...
optlist, files = getopt.getopt(sys.argv[1:], 'hqpc:t:k:n:g:d:l:r:w:sS:B:GAD:Os:H3:', \
		...
		'orland','seq=','fixrep','pdb','kh','pasta','nocharmm',
...
if kh and pasta:
	sys.stderr.write("Can't use both KH and PASTA matrices together!!\n")
	sys.exit(1)
...
for i in range(nchain):
        ...
        if not use_list:
    	    ...
    	    if kh==1:
    	    	...
    	    	khmap[chaini][chaini] = go_lib.calc_pasta_intra(meta_crd[chaini],E0_PASTA,LAMBDA_PASTA)
    	...
    	for j in range(i+1,nchain):
    		...
    		if kh==1:
    			...
    			khmap[chaini][chainj] = go_lib.calc_pasta_inter(meta_crd[chaini],meta_crd[chainj],E0_PASTA,LAMBDA_PASTA)
...
elif pasta==1 and charmm:
	go_lib.write_parms(outp_base+"_pasta_parm.inp",key,bonds,angles,dihedrals,
		nonbonded['sig'],GOMODEL_nbfix_kh,NSFAC,tfbb,winte,0)
	go_lib.write_parms(outp_base+"_onlypasta_parm.inp",key,bonds,angles,dihedrals,
		nonbonded['sig'],globalkh,NSFAC,tfbb,winte,0)
...
if nchain > 1 and charmm:
	if kh==1:
		...
		go_lib.write_parms(outp_base+"_pasta_parm_inter.inp",key,bonds,angles,dihedrals,
				nonbonded['sig'],GOMODEL_nbfix_kh_inter,NSFAC,tfbb,winte,0)
		go_lib.write_parms(outp_base+"_pasta_parm_inter_nongo.inp",key,bonds,angles,dihedrals,
				nonbonded['sig'],GOMODEL_nbfix_kh_inter,NSFAC,tfbb,winte,0)
		if ncrowd > 0:
			go_lib.write_parms(outp_base+"_pasta_parm_inter_crowd.inp",key,bonds,
				angles,dihedrals,
				nonbonded['sig'],GOMODEL_nbfix_kh_crowd,NSFAC,tfbb,winte,0,
				0)
	#
...
if (gmx):
	...
	if kh==1:
		...
		# KB for all go and KH for all other pairs
		go_lib.write_gro_top(outp_base+"_gromacs_gopasta.top",key,residues,unmerged_bonds,
				unmerged_angles,unmerged_dihedrals,nonbonded['sig'],
				go_map,NSFAC,tfbb,seqs,symm,winte,"12_10_6",globalkh,addcharges,1,n_repeat)
		# KH for all pairs
		go_lib.write_gro_top(outp_base+"_gromacs_onlypasta.top",key,residues,unmerged_bonds,
				unmerged_angles,unmerged_dihedrals,nonbonded['sig'],
				{},NSFAC,tfbb,seqs,symm,winte,"12_10_6",globalkh,addcharges,1,n_repeat)
		if nchain > 1:
			# Go for all intramolecular go pairs and KH for all other pairs
			go_lib.write_gro_top(outp_base+"_gromacs_intragopasta.top",key,residues,
					unmerged_bonds,
					unmerged_angles,unmerged_dihedrals,nonbonded['sig'],
					go_map_intra,NSFAC,tfbb,seqs,symm,winte,"12_10_6",globalkh,
					addcharges,1,n_repeat)
			# Go for all intramolecular go pairs, repulsive intramolecular
			# nongo pairs and KH for all other pairs
			go_lib.write_gro_top(outp_base+"_gromacs_intragoreppasta.top",key,residues,
					unmerged_bonds,
					unmerged_angles,unmerged_dihedrals,nonbonded['sig'],
					go_map_intra,NSFAC,tfbb,seqs,symm,winte,"12_10_6",globalkh,
					addcharges,0,n_repeat)
